...
<body>
  ...
  <main>
    <section>
      <h1>Wanneer komen er veel mensen?</h1>
  <?php
  try {
   $db = new PDO("mysql:host=localhost;dbname=van_raam","root","");
  
 $query = $db->prepare ("SELECT begindatum, einddatum, onderhoud.idonderhoud as idonderhoud
FROM `onderhoud` 
INNER JOIN fiets ON onderhoud.fietsid=fiets.fietsid 
INNER JOIN contract ON fiets.fietsid=contract.fietsid");
  $query->execute();
