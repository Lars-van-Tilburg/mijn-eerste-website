...
<body>
  ...
  <main>
    <section>
      <h1>Hoeveel reparaties per merk fiets?</h1>
  <?php
  try {
   $db = new PDO("mysql:host=localhost;dbname=van_raam","root","");
  
 $query = $db->prepare ("SELECT COUNT(onderhoud.fietsid), soort 
 FROM onderhoud INNER JOIN fiets ON fiets.fietsid=onderhoud.fietsid GROUP BY soort");
  $query->execute();
 ...
 {
 echo    "<tr>";
 echo    "<Td>" .$data["COUNT(onderhoud.fietsid)"]."</td>";
