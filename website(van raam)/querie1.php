...
<body>
  ...
  <main>
    <section>
      <h1>Welke merken hebben wij in onderhoud?</h1>
  <?php
  try {
   $db = new PDO("mysql:host=localhost;dbname=van_raam","root","");
  
 $query = $db->prepare ("SELECT  fiets.fietsid, soort, idonderhoud
 FROM fiets
 INNER JOIN onderhoud
  ON fiets.fietsid=onderhoud.fietsid");
  $query->execute();
