...
<body>
  ...
  <main>
    <section>
      <h1>Verzekerd ja of nee?</h1>
  <?php
  try {
   $db = new PDO("mysql:host=localhost;dbname=van_raam","root","");
  
 $query = $db->prepare ("SELECT voornaam, tussenvoegsel, achternaam, verzekering FROM contract INNER JOIN klant ON klant.klantid = contract.klantid ");
  $query->execute();
